...
func handleEliza(w http.ResponseWriter, r *http.Request){
	// Create a new instance of Eliza.
	eliza := ElizaFromFiles("data/responses.txt", "data/substitutions.txt")
	
	elizaResponse := r.URL.Query().Get("value")

	fmt.Fprintf(w, "Shane: %s!\n",elizaResponse) 
	fmt.Fprintf(w, "Eliza: %s!\n",eliza.RespondTo(elizaResponse))
...
// https://github.com/data-representation/eliza adapted from this 